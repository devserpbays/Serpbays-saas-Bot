...
export default function SettingsPanel({ open, onClose, workspaceId, role = 'owner' }: SettingsPanelProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex justify-end" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="w-full max-w-lg bg-white h-full overflow-y-auto p-6 space-y-6 shadow-2xl animate-[slideIn_0.2s_ease-out]">
        ...
        {canEdit && (
          <div className="flex items-center gap-3 pt-3 pb-4 border-t border-gray-100 sticky bottom-0 bg-white -mx-6 px-6 -mb-6 shadow-[0_-4px_12px_rgba(0,0,0,0.05)]">
            <button onClick={handleSave} disabled={saving}
              className="px-5 py-2.5 bg-green-600 text-white text-sm font-medium rounded-lg hover:bg-green-700 disabled:opacity-50 transition-colors flex items-center gap-2 shadow-sm">
              {saving ? (
                <>
            ...
            {message && (
              <span className={`text-sm font-medium flex items-center gap-1 ${message.startsWith('Error') ? 'text-red-600' : 'text-green-600'}`}>
